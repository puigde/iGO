...
# Keys of the dictionary necessary to store igraph
# and the time in the dictionary graph_info
key_time = random.randint(1000000, 9999999)
key_graph = random.randint(1000000, 9999999)

# i_graph and the time when it has been made are stored
# in a dictionary called graph_info
graph_info = {key_graph: i_go.prepare_i_graph(), key_time: datetime.now()}
...
def author(update, context):
    ...
    context.bot.send_message(
        chat_id=update.effective_chat.id,
        text="Made by Pol Puigdemont Plana and Jan Sallent Bayà",
    )
...
def go(update, context):
    ...
    try:
        ...
        destination += ", Barcelona"  # if we add ", Barcelona" at the end,
        # geocode function becomes much more accurate
